...
---
## Mobile functional closure — honest reads, races, stock authority (2026-09-26)
- D1 FIXED: `viewmodels/home/homeComposer.ts` was dead (no importer) with hardcoded zero signals and mirror-based stock — deleted with its three helpers. D2 NOT A DEFECT: `/restock` behind `purchasing` = ADMIN+ACCOUNTANT exactly matches the live purchase write policies; `stock` would have broadened it. D3: tsc 0. D4: already wired. D5: courier-store work + dist-mobile left uncommitted.
- Fixed: Home served refresh/retry from a tab-lifetime cache, pinned failed reads, and leaked the previous user's store data across sign-out; paged lists and detail screens dropped a new search/record while an older read was in flight and painted the stale answer; Product Details showed `products.quantity` (live: 24/51 vs ledger 22/60); five fake-zero/empty-on-failure reads; deleted orders in the customer summary and waiting orders; Shipments discarded courier-search results; the restock deep link spun forever on a failed or stale id; customer history load-more skipped pages; stock filters claimed "none" with pages unloaded; shortages kept a stale total beside an error.
- Verified: `check_mobile_functional_closure.mjs` (17 tests; the 4 behavioural ones run the real hooks through a hooks harness and fail on the old hooks). Three tests that pinned the old guard lines re-expressed, not loosened. Working tree 1405 / 1399 / 0 fail / 6 skipped; tsc 0; both builds green. No Supabase change.

---
## Desktop P1 closure — authorization, types, and the failed-read sweep (2026-09-26)
- G-14 FIXED (consistency): `canReturnWholesale` = ADMIN, the intersection of the three live policies a trader return writes. `commitWholesaleReturn` asks it as step 0 of the transaction, before any write; `/orders` shows the refusal and disables the button. Runtime (real command chain, network controlled, no credentials exist to sign in): ADMIN completes with exactly records → invoice → ledger; POS/ECOM/ACCOUNTANT/MODERATOR make ZERO writes and the invoice is untouched. Whether POS/e-commerce staff should complete these is still a business decision (§G-14); nothing was widened.
- P1-6 FIXED for the financial core: 66 → 45 `any` domain types, every replacement from the live schema or an existing canonical type (ten ledger types had been re-declared as `any`, shadowing the real ones). The 45 kept are classified with reasons; `Product` is deferred as a structural refactor, its bug class pinned by test instead.
- Real defects the types exposed, all fixed: (1) a wholesale shipment could be SOLD TWICE — delivering it in wholesale mode appended a second `sale` and a second invoice; it is now linked to its invoice and delivery moves the document only; (2) its delivery address went to `customerAddress`, a field the whitelist drops; (3) items added while editing a pending order read `product.price` / `product.cost` — neither exists — so they went in at price 0 and cost 0; (4) courier statement reprints matched `codSettlementId`, which has no column, and listed no orders; (5) a customer created from an address-less order was refused by NOT NULL. Smaller: `client.name`, «Invalid Date» on discount codes, null-typed helpers.
- Failed-read sweep (P1-12): every money mutation that consults a read was classified. One FIX REQUIRED — the expense budget cap was checked AFTER the ledger event (an over-cap expense was paid, then "refused", gate left held), against a list that reads 0 before it loads. Now checked first, with `spending_unknown` as its own answer. The owner budget card no longer shows a failed read as a full, green budget.
- I-2 / I-3 CONFIRMED and documented, NOT applied (SUPABASE NOTES in the audit, §I): a POS_ECOMMERCE user can append a line to an existing ADMIN event — +1,000,000 EGP minted into a till in a rolled-back probe — and can re-key a product's `id`. Exact policy/trigger changes given; the I-2 predicate was dry-run on the live database (true for a fresh `ledger_append`, false for the exploit). Production migrations required.
- M-2 re-verified: operational, fail-closed, not an application bug. O-2 partially fixed: `.github/workflows/ci.yml` verifies the committed tree (tsc, suite, build) with no secrets; the live tests need a QA project (F-18). Expenses LWW re-verified latent P2.
- Verified: +19 tests; 19 mutations, 19 caught (one escaped first — a real test defect, fixed). Working tree 1388 / 1382 / 0 fail / 6 skipped; tsc 0; both builds green.

---
## Wholesale return — transaction integrity on committed main (2026-09-26)
- Problem: since `dccf949`, the three wholesale-return screens (جملة, نقاط البيع, الطلبات) called `useBusinessStore.getState().recordWholesaleReturn(...)` AFTER `commitWholesaleReturn` had appended the ledger event — and the method was never committed. On `main` it threw a TypeError after the money and stock had moved, and the screen said «لم يُسجَّل المرتجع ولم يتغيّر أي رصيد». With the method present, the call was `.catch(() => {})`: a refused invoice write left the ledger and the invoice «متبقي» disagreeing, silently. Found by isolation-testing the P1 Wave 2 commit; all earlier tsc/suite figures had been measured on the working tree.
- Fix: the credit moved INSIDE the canonical command and BEFORE the ledger event — records, then invoice credits, then the event last, so nothing that can fail runs after an append-only write. Order and undo in `lib/wholesaleReturnTxn.ts` (import-free). A refusal restores each credited invoice to the EXACT balance it held (the clamp at 0 makes "add it back" wrong), deletes the records and rethrows — the screens' existing message becomes true. A failed undo throws `WholesaleReturnUndoError` naming what was left. The store method throws on a missing invoice instead of returning quietly; the three post-ledger loops are gone. `OrdersPage` now says «المرتجع اتسجّل…» if only the order's own status write fails afterwards.
- Surfaced, not changed (§G-14): live RLS lets only ADMIN write all three things a trader return needs. A POS_ECOMMERCE / ECOMMERCE_ONLY trader return on `/orders` used to half-post; it is now refused and rolled back completely (their invoice upsert is refused 42501, proven live). No Supabase change.
- Runtime, live and self-aborting: ADMIN credits an own-store invoice (1 row); ADMIN crediting another tenant's invoice touches 0 rows; POS_ECOMMERCE's upsert refused 42501. Nothing persisted.
- Verified: new `check_wholesale_return_txn.mjs` (15 tests) drives the transaction with a fake ledger/invoice/record world — order, exactly-once ledger, per-invoice credit at the paid price, the ceiling refusing a resubmission, the order claim refusing a double click, and every failure direction (records, credit, ledger, undo) leaving no ledger movement. 11 mutations, 11 caught. One pre-existing assertion repointed to the command, stricter. Working tree 1369 / 1363 / 0 fail / 6 skipped; tsc 0; both builds green.

---
## Desktop P1 Wave 2 — state correctness and financial authority (2026-09-26)
- P1-D Problem: two readers answered "nothing" when it was not nothing. Hydrated stores start empty, so a list said «لا توجد …» while loading AND after a failed read; `useBalances` answers 0 before its read lands AND after it fails, so money figures printed «٠ ج.م» for reads that never happened.
- P1-D The worse half, found in this sweep: five kinds of COMMIT decided on those zeros. A supplier return and a trader return (جملة, نقاط البيع, الطلبات) settled against an unread debt of 0 post as a cash refund instead of clearing the balance; a جرد against an unread `qtyOf() = 0` books phantom surplus; a dividend with unread draws of 0 pays a working partner twice; and a failed settings pull let one keystroke push «محلي» and blanks over the real store name, phone and tax number (`updateSettings` pushes on every edit). Plus three PDFs that printed zeros, and an opening-balance prompt that invited a duplicate entry. All refuse now, with a retry.
- P1-D Model, reused everywhere: `lib/figure.ts` (`statusOf`: error outranks loading; `moneyFigure`/`figureOr`: «— ج.م» on error via the project's own `formatMoney(null)`, «…» while loading, the number only when every read under it succeeded); per-table status in `useSyncStatus`, recorded by `hydrateAll`; `hydrateTable`, the loop body turned into the one reader boot and retry share (no second fetch path, empties nothing else, in-flight dedupe); `CollectionGate`, wrapping only a list's EMPTY branch; `LoadError`, the dashboard's existing banner lifted out, with a retry that cannot be double-fired. `useBalances` now reports a recovering read as loading — but not a routine refresh, which would blank the till after every sale.
- P1-D All 21 gated screens covered; Preferences, Integrations and Backups have no cloud read. Not gated on purpose: the courier LIST, whose hydrate sink is in the separate uncommitted courier work — a gate on a table the committed hydrate never marks ready would spin forever. Also fixed: the returns log read `getState()` in render and never re-rendered when the hydrate landed.
- P1-E Finding: `owner_financial_summary` is built on `ledger_balances`, the same SQL function Desktop's `balances()` calls — one aggregation authority. The duplication was the arithmetic on top: net profit written four times (RPC, `pnl()`, `summarise()`, الشركاء inline) and the counting logic pasted a fifth time into the dashboard by the interrupted attempt.
- P1-E Fix: نظرة عامة (ADMIN-only) consumes the RPC and computes no money figure; everything else goes through ONE client definition, `netProfitOf`; counts through ONE `windowCounts`. Moving الشركاء onto the RPC was checked and rejected: migration 034 refuses ACCOUNTANT by decision and says ACCOUNTANT keeps `useBalances`, and `/partners` is theirs too. No SQL changed; no second SQL function.
- P1-E Cross-check, live, QA-STORE (disposable), real ADMIN impersonated: the RPC vs the exact `ledger_balances` calls Desktop makes — 13 metrics × 5 windows = 65 comparisons, 0 mismatches (lifetime net 19893 piastres). Plus a 2,001-case sweep: SQL's integer piastres and the client's EGP-float arithmetic display identically. The production tenant cannot be crossed through the RPC — it has no members (M-2, unchanged).
- Runtime, in-app browser: no QA credentials exist in the project and none were invented, so a temporary uncommitted page mounted the REAL gate, banner, `useBalances`, `hydrateTable` and Owner hook with only the network controlled. Failed → banner + retry, `— ج.م` (never `٠`), net profit withdrawn; double-click → 1 request per reader; in flight → skeleton, buttons disabled; recovered → `٧٬٥٠٠ ج.م` and `١٩٨٫٩٣ ج.م`, exactly SQL's 750000 and 19893; success with 0 rows → the honest empty message.
- Found at runtime and fixed: the Owner hook's `void promise.finally(...)` re-rejected into nothing — every failed read logged "Uncaught (in promise)". Now `promise.then(clear, clear)`; re-proven with 2 failed reads, 0 unhandled rejections.
- Expenses LWW (P1 Wave 1 finding) verified and placed as P2 F-14: the handler does merge unconditionally, but expense rows are insert-once and hard-deleted and nothing updates one, so nothing triggers it today; money is ledger-derived regardless. One-line fix documented; not applied, so P1-C's handlers are not reopened for an untriggered risk.
- Verified: +45 tests (`check_load_states.mjs` 32, `check_owner_authority.mjs` 13); 32 mutations, 32 caught, every file verified byte-identical afterwards. Suite 1354 / 1348 pass / 0 fail / 6 skipped — no pre-existing test needed changing. `npx tsc --noEmit` 0, `npm run build` 0, `npm run build:mobile` 0. Realtime untouched (auth gate, one channel, 13/16).

---
## Desktop P1 Wave 1 — numbering, stock authority, realtime coverage (2026-09-22)
- One theme in three places: the Desktop trusting something LOCAL for an answer only the server has. A device clock for a document number, a per-browser cache for "is this in stock", and a subscription list written by hand while the publication grew past it.
- P1-A Problem: every order was numbered `ECO-${Date.now()}`, in the browser, at two sites (`routes/ecommerce-orders.tsx:749` and the `useOrderStore.addOrder` fallback). That is a timestamp wearing a document number: drawn from the DEVICE clock, so a till a day behind issued numbers sorting before yesterday's orders; two devices in the same millisecond collided and `orders` had NO uniqueness on the number to refuse the second; and thirteen digits of milliseconds is not something anybody reads down a phone.
- P1-A Fix: both sites draw `nextDocumentNumber("ecommerce_order", "ECO-")` — the counter `FJ-`, `FM-` and `SP-` have used since migration 016. Migration 042 adds `orders_number_per_store (store_id, "orderNumber")`, the same last line of defence `wholesale_invoices` and `purchase_invoices` already had. `next_document_number` itself needed NO change: the live definition already admits exactly ADMIN / POS_ECOMMERCE / ECOMMERCE_ONLY / ACCOUNTANT, the set of roles that may take an order. (The repo copy of 016 still shows the older `is_store_member`; the live database is authoritative.)
- P1-A History is not renumbered. The 22 existing `ECO-<13 digits>` orders keep the numbers they shipped and were invoiced under. The counter seeds from canonical numbers only (`^ECO-[0-9]{1,9}$`), so all six stores start at 0. Seeding the way 016 does — `MAX` of every digit in the column — would have read a millisecond timestamp and started the sequence at 1.75 trillion, putting the clock back into the numbering one increment at a time. The two shapes cannot collide: 8 characters versus 17.
- P1-A A refused order still burns a number. Gaps, not duplicates — the same trade جملة and الشراء already make. A gap is a question someone can answer; a duplicate is a document nobody can trust. A failed allocation returns the discount use that was claimed for it, so a refused order never leaves a one-use code burnt.
- P1-A `addOrder` REFUSES rather than throwing, in the `{ success: false, reason }` shape the callers already handle — the جملة caller does not wrap it in a try, and an unhandled rejection there would be silence.
- P1-A Runtime, self-aborting DO block on the live project impersonating a real ADMIN and a real POS_ECOMMERCE via `request.jwt.claims`: three draws gave ECO-0001/0002/0003, distinct and monotonic; store A advanced by exactly 2 while store B did not move; the legacy `ECO-1789426501524` (17 chars) cannot meet `ECO-0001` (8); a duplicate within a store was REFUSED by the index; POS_ECOMMERCE drew ECO-0004; a non-member was refused 42501; and a client can read 0 rows of `store_counters`, so the sequence cannot be rewound from a browser. Nothing persisted — 321 events / 678 lines / 36 orders / 141 products unchanged, 0 probe rows, every `ecommerce_order` counter back at 0.
- P1-B Problem: `getActualStock` prefers the ledger but falls through to the `products.quantity` mirror when no aggregation has landed. Right for RENDERING — `null` means "not loaded yet", and turning it into 0 paints a sold-out shop on every cold start — and wrong at the three places that COMMIT against stock, which could not tell a ledger number from a mirror number. A till that had not hydrated since another till sold the last six units still held six, and would accept the order.
- P1-B Classification: only two live `products.quantity` readers remained. `stockMirror.ts:153` is the mirror's own writer (legitimate); `product.ts:168` was the defect. Mobile never reads it, valuation is ledger-derived, and `StockAuditPage` reads it deliberately as the thing being audited.
- P1-B Fix: `stockIsAuthoritative()` says whether an aggregation has landed. The POS cart, the جملة invoice and the online order form ask first and refuse with «المخزون لسه بيتحمّل من السحابة» rather than validate against a number they cannot vouch for. The mirror is NOT deleted — it is what lets 200 products render without 200 aggregations — and the ~48 display callers are untouched, asserted in both directions.
- P1-B Measured, and this CORRECTS the earlier reading in the audit which did not separate the tenants: production (المحل التجاري) has 0 disagreements across 132 products with ledger lines; QA-STORE has 3 of 6 (QA-UAT-PROBE2 0/14, QA-UAT-WIDGET 24/22, غسول سيرافي 51/50). The drift is demonstrated to occur and has NOT occurred in production. This closes the path; it repairs no live number, and nothing was written to correct the QA drift — the mirror is derived and rebuilds itself.
- P1-B Not attempted: a database-level oversell guard. Backorders are legitimate — the order form skips the stock check for backordered lines and تقرير النواقص sums the deficit — so a constraint refusing a negative `SUM(qty_delta)` would refuse real business.
- P1-C Problem: 16 tables are in `supabase_realtime` and 5 were listened to. The other 11 were published and then ignored, which is the worst of the two states — the write is broadcast to every tab and every tab drops it. No error, no indication, just a second device showing yesterday's data.
- P1-C Fix: 8 added, each because a real Desktop screen reads it and a second device can change it while that screen is open — customers, suppliers, purchase_invoices, return_records, discount_codes, wholesale_clients, wholesale_invoices, shipping_rates. 3 excluded WITH REASONS in the source: `ledger_lines` (arrives with its event; `ledger_events` already pulses the readers, and subscribing re-fires that once per line of every sale), `stores` (licence and identity, owned by `useSessionReconciliation`), `branches` (structural). `couriers` is not in the publication at all.
- P1-C The listeners are now built by reducing over `TABLE_HANDLERS` instead of being written out by hand. In source, a handler nobody subscribes to is indistinguishable from a working one — which is exactly how four listeners stayed the whole of realtime while the publication grew to sixteen. Still ONE channel. The eight new handlers come from one factory: eight hand-copied merges would be eight chances to get Last-Write-Wins subtly different, which `orders` (camelCase `updatedAt`, parsed as a Date) and `products` (snake_case `updated_at`, compared as strings) already are. The four original handlers were left exactly as they were.
- P1-C The auth gate from `1cb38ce` is preserved and pinned: the channel opens only under `isCloudSyncMode() && authenticated`, the boot hydrate still returns early on `!authenticated`, and the hook still returns `sessionState` to `ProtectedRoute`. Three separate mutations confirm each.
- Verified: new `check_desktop_p1.mjs`, 13 tests. TWENTY-TWO mutations, all caught. Two ESCAPED first and both were real test defects: the "ledger wins" test pinned source ORDER, which survives falsifying the condition around the ledger read (now pins the condition and the return); and the realtime table test grepped for table NAMES, which survive inside a call whose result is discarded (now asserts the handlers are spread into the map). The second was caught the hard way — a crashed mutation run left the mutated file on disk and the suite still passed on it.
- Three PRE-EXISTING tests failed mid-wave and were repointed, not weakened; each pinned a real invariant through a marker this wave replaced. `check_order_traceability` · *allocated BEFORE the ledger event* now finds the `nextDocumentNumber` call and additionally asserts the route mints nothing itself. `check_order_traceability` · *the store uses the caller's number* now asserts the fallback is the same counter, on comment-stripped source. `check_sync_layer` · *every subscribed table is published* now reads `TABLE_HANDLERS` itself instead of a hand-written list, scans EVERY migration for the publication rather than only the init file and 036, and has its floor raised from 5 to 13 so losing coverage again fails loudly.
- Suite 1309 total / 1303 passed / 0 failed / 6 skipped. `npx tsc --noEmit` 0, `npm run build` 0.
- Untouched by design: `products.quantity` as a mirror, historical order numbers, production financial records, Mobile, the `expenses` handler's missing LWW guard (pre-existing, carried forward as P2), and `CheckoutForm`'s `pos_<Date.now()>` — a synthetic foreign key for a walk-in return with no originating order, not a business document number.

---
## Auth — desktop login regression (2026-09-22)
- Symptom: on Desktop, correct credentials did nothing. The login screen accepted them, the session was created, and the app came straight back to /login.
- Root cause, and it was mine — introduced in 11ebc67. `useSessionReconciliation` resolves ONCE in a `useEffect` with `[]` deps; after that the only thing that could move its state was the `onAuthStateChange` listener, and that listener was one-directional: `if (!session && (event === "SIGNED_OUT" || event === "TOKEN_REFRESHED"))`. A `SIGNED_IN` event ALWAYS carries a session, so it matched nothing. A boot with no session resolved to "unauthenticated" and stayed there until a page reload. That was invisible while the return value was discarded; once `ProtectedRoute` began gating on it (`sessionState !== "authenticated" || !isAuthenticated`), signing in set the store flag, navigated to /, and the guard bounced it back.
- Mobile was NOT blocked, and the difference is instructive: `MobileSessionGate` tests `sessionState === "unavailable" || !isAuthenticated`, never `!== "authenticated"`, so a stuck "unauthenticated" passed straight through. What mobile lost instead was silent — `useMobileRealtime(sessionState === "authenticated")` never turned on after a fresh sign-in, so the app ran without realtime until the next reload. One root cause, two surfaces, one loud and one quiet.
- Fix: the watcher now reports a session that APPEARS as well as one that vanishes — `if (session) { setState("authenticated"); return; }` ahead of the existing sign-out branch. That is not a privilege grant: it says only that Supabase currently holds a session, which is the fact this hook exists to track. `ProtectedRoute` still requires `isAuthenticated` too, and that is set by `establishSupabaseSession` only after membership has resolved; the next boot still runs the full membership check and signs out anyone whose membership has gone. No second auth path, no localStorage workaround, no timeout, no route bypass.
- Runtime, against the live QA ADMIN session in the dev browser, reproducing the exact login sequence without a password by hiding the session, booting cold, then re-establishing it through the app's own `setSession` + `establishSupabaseSession` — the same post-authentication path `signInWithPassword` calls. PRE-FIX: session established (`ok: true`, no error) and the app stayed on /login. POST-FIX: identical sequence, `isAuthenticated: true`, role ADMIN, and a client-side navigation to /inventory rendered the real screen (6 products, 8,876.43 ج.م stock value). F5 on /inventory kept the session; /orders and /partners both rendered. Sign-out cleared the flag AND removed the Supabase token, Back could not restore it, and a direct protected URL redirected to /login.
- The five long-standing "stale auth failures" were NOT pre-existing noise and are now gone. Each pointed at code that the mobile refactor had moved, and Phase 14 of this task required checking rather than assuming. All five were REBASED onto the canonical source and none was deleted or weakened: two in `check_online_only` now read `lib/auth/useSessionReconciliation.ts` and additionally assert that `useRealtimeSync` still MOUNTS it, so an extraction cannot become a deletion; `check_no_login_backdoor` asserts `signInWithPassword` in `sessionWorkflow` AND that the screen routes through it, with the no-literal-credentials rule applied to both files; the two `check_invite_staff` tests read `completePasswordSetup`, keeping the confirm-field comparison on the screen where the second field actually exists.
- Suite 1296 total / 1290 passed / 0 failed / 6 skipped — the first fully green run of this engagement. New `check_login_regression.mjs` (10 tests) was written FAILING first and passes only with the fix. Twelve mutations all caught, including reinstating the regression, reporting authenticated with no session, dropping either half of the guard, rendering while still checking, swallowing the verdict again, ungating mobile realtime, putting the membership read back behind the local flag, signing out a store-less System Owner, and making ownership persistable.
- `npx tsc --noEmit` 0, `npm run build` 0, `npm run build:mobile` 0. Certified-area subset 426 tests / 422 passed / 0 failed / 4 credential-skipped.
- Note for whoever uses that dev browser: verifying sign-out revoked the QA session, so `shahd…@gmail.com` has to log in again there.

---
## Returns — courier claim settlement integration (2026-09-22)
- Task: connect `courier_claims` to the EXISTING courier settlement workflow so a claim can actually reach `settled`. No second settlement system, no second money store, no UI-only status.
- Reuse, not replacement: حسابات الشحن settles a courier exactly as before — one `courier_settlement` event, the same `buildCourierBatchSettlementLines`, the same receivable cleared. The screen now keeps the event id it was already producing and offers that courier's APPROVED claims to be closed with it. Ticking a claim adds nothing to the transfer: the receivable it represents is already inside the balance being settled.
- The UI cannot set `settled`. There is no status control for it; the only call that names `settled` passes the id of the event that just moved the money.
- Migration 040: `settled` now requires an event that is a `courier_settlement`, in THIS store, carrying a `receivable_courier` line for THIS courier — asked of the ledger LINE rather than a label, because the batch path puts the courier in the payload and the per-order path does not name it at all. Neither `ref_id` nor the payload is a reliable place to ask; the money is.
- Migration 041, and this one was found by PROBING 040 rather than reading it: the guard returned early whenever the status was unchanged, so a settled claim's `settlement_event_id` could be re-pointed at a `sale` event and its `amount_piastres` rewritten to anything. The amount is the snapshot the ledger line is reconciled against — one that can be edited afterwards reconciles with anything. 041 freezes order, courier, return record, amount, settlement event AND the status itself on a terminal claim, checked BEFORE the early return it was hiding behind. Only `notes` and `deleted_at` stay writable.
- Failure direction: the event is written first, claims closed after. If the claim half fails the money has still landed correctly and the claim stays `approved`, recoverable next transfer — and the operator is told. The reverse order would mark a claim settled against an event that might never be written.
- Runtime, self-aborting DO blocks against the disposable QA store with real roles impersonated via `request.jwt.claims`: A claim=pending; B submitted + submitted_at; C approved + decided_by; H a CASHIER's update matched 0 rows (RLS hid it) and their insert was refused by the policy; I a settlement belonging to ANOTHER store refused; D settled with settled_at and the event linked; E reopen refused `NEXUS_CLAIM_IS_CLOSED`; F deposit still held 30000 with 0 refunds and the order untouched; G after the deposit refund the claim was STILL settled with amount and event unchanged; J a wrong-courier and a non-settlement event both refused `NEXUS_CLAIM_SETTLEMENT_EVENT_MISMATCH`; K amount frozen after close and a zero amount refused by the CHECK. Nothing persisted — 321 events / 678 lines, unchanged.
- Verified: `check_cancellation_cause.mjs` now 35 tests. TWELVE mutations all caught, including all eight the brief named. Four initially ESCAPED and each was a real coverage defect, not a false alarm: three because the lifecycle assertions were reading migration 039 after 041 had REPLACED the guard function — they would have stayed green while the live transition table was broken — and one because nothing pinned the status freeze on a terminal claim, where the early `RETURN NEW` makes the `ELSE false` below unreachable. All four assertions were repointed or added.
- Suite 1286 total / 1275 passed / 5 stale pre-existing auth failures / 6 skipped — zero new. Certified-area subset (mobile, RLS anon, moderator, system owner, licence, order lifecycle, owner financials) 251 tests / 249 passed / 0 failed / 2 credential-skipped. `npx tsc --noEmit` 0, `npm run build` 0, `npm run build:mobile` 0.
- Deposit policy, return causes and exchange responsibility are untouched by this change.

---
## Returns — owner decision: the resolution is TWO decisions (2026-09-22)
- Owner decision, final: a courier- or shop-caused return NEVER refunds the deposit automatically. It is held, and the EMPLOYEE chooses the customer resolution. This also closes what the previous pass flagged as open — shop-caused follows the same non-automatic principle as courier, so the extension that was flagged for confirmation is now the rule rather than an inference.
- Problem found against the decision: the resolution offered رد العربون on the SAME click that identified the courier as the cause. The button read «تسوية العميلة — رد العربون» and the dialog went straight to the refund form. Identifying the courier says nothing about whether the customer still wants the goods — most of the time they do — so that made the refund the default answer to a question nobody had asked them yet.
- Fix: تسوية العميلة is now two steps. Step one offers «إنشاء طلب بديل — العربون يفضل محجوز» and «إنهاء الطلب ورد العربون»; the refund form, its wallet picker and its confirmation live behind the second. Backing out goes back to the choice and writes nothing. Both steps state that the shipping claim is independent — step one because the operator is choosing, step two because that is the moment money moves.
- Resolution A moves NO money: it navigates to the order-entry screen with `?exchangeOf=`, which writes `original_order_id` on Order B. Order A keeps its status, its cause, its claim and its held deposit. Asserted as an absence — the navigation helper contains no `refundOrderDeposit`, no `appendEvent` and no builder call, and the entry screen cannot refund at all.
- Verified: 8 new tests in `check_cancellation_cause.mjs` (26 total) covering the two-step requirement, both resolution labels, Resolution A moving nothing, the entry screen linking rather than editing, and the absence of any code path from a courier cause to an automatic refund — the ledger builders never read a cause, and no policy function may invoke the refund. Twelve mutations still all caught, including the one the owner named: restoring the automatic courier refund fails 12 tests.
- One PRE-EXISTING assertion needed updating: `S5 · declining the refund writes nothing` pinned the button text «إلغاء — نحتفظ بالعربون», which became «رجوع» when backing out started returning to the choice instead of closing the dialog. It now matches the promise («نحتفظ بالعربون») and the handler, rather than the button it happened to sit on.
- Suite 1278 total / 1267 passed / 5 stale pre-existing auth failures / 6 skipped — zero new. `npx tsc --noEmit` 0, `npm run build` 0, `npm run build:mobile` 0.

---
## Returns — cancellation cause, claim lifecycle, shop-case audit (2026-09-22)
- GAP 1 — Problem: a courier that fails a delivery may report "the customer cancelled". `cancelOrder(orderId)` took no cause and recorded none, so the falsified cancellation and the real one were the SAME ROW — and both forfeited the customer's deposit. Fix: إلغاء الطلب asks, offering العميلة لغت بنفسها / المندوب / خطأ من المحل, and writes the answer to the order AND to the append-only `order_cancelled` payload. `customer` forfeits (Rule A); `courier` and `shop` HOLD and open the incident path. Neither auto-refunds.
- GAP 1 — Server authority: `orders_guard_return_cause`, a TRIGGER not an RPC, because the cause is written from four handlers and from anything anyone points at PostgREST tomorrow — an RPC guards the path that calls it, a trigger guards the table. `courier` and `shop` require ADMIN or ACCOUNTANT, the same pair the ledger's money kinds require; `write_orders` admits POS_ECOMMERCE and ECOMMERCE_ONLY, so without this a cashier could assert that a shipping provider owes the shop money. `customer` stays open — it creates nothing to claim. And the cause FREEZES once a `deposit_refunded` event exists, because re-pointing the blame after the money moved leaves a refund standing on an order that no longer justifies it.
- GAP 1 — Workflow impact, stated rather than buried: a POS_ECOMMERCE or ECOMMERCE_ONLY user can no longer confirm a return as courier- or shop-caused. That is the intended restriction and it is a real change to who can finish that step.
- GAP 2 — `courier_claims`: pending → submitted → approved → settled, with `rejected` reachable from the first two and both terminal states enforced by a trigger. A status column with no transition rule is a column where any state reaches any other, which is not a lifecycle. Traceable to order, courier, return record, amount, settlement event, four dates and two users — by FOREIGN KEY, not text ids. One live claim per order via a partial unique index, so a duplicate is refused by the database rather than by reading first and hoping.
- GAP 2 — It holds NO money, deliberately. The receivable already lives in `ledger_lines` and the settlement is a `courier_settlement` event this table only REFERENCES; `amount_piastres` is a snapshot for reconciliation and nothing sums it, because a second summed amount is how two answers to "what does this courier owe us" come to exist. It is equally not a string in a ledger payload: `no_update_ledger_events` is `USING (false)`, so a status living there could never advance past the moment it was written. A claim cannot reach `settled` without a real settlement event id.
- GAP 3 — Audited rather than assumed. The evidence DOES determine that `shop` is not forfeited: `RETURN_CAUSE_HINTS.shop`, the old `depositForfeitedOn` doc and migration 029 all say so. It does NOT determine refund-now versus hold, because the one source that said "refunded" is the same boolean this correction overrode for the courier case. So `shop` stays HELD — the only answer that moves no money and leaves both outcomes reachable — and the decision is documented as still open, with no Settings toggle invented for it.
- GAP 3 — Found while auditing: two shipped hint strings still promised «والعربون يرجع للعميل» (the deposit goes back), which stopped being true the moment the disposition became "held". Both corrected. Shipped copy that contradicts the ledger is worse than no copy.
- Runtime, against the disposable QA store inside a self-aborting DO block, impersonating REAL roles via `request.jwt.claims` so RLS and the trigger both applied: the CASHIER was refused `NEXUS_CAUSE_NOT_AUTHORISED` on cause=courier and allowed cause=customer; the ADMIN recorded courier and the deposit came out held=30000 with ZERO wallet lines; Order B linked to A with A untouched; settle-without-event refused, a backwards transition refused, a duplicate claim refused; the claim settled and the courier receivable netted to 0 while the deposit was STILL held at 30000; then the refund paid wallet=−30000, held→0, exactly one `deposit_refunded` event, and the claim was STILL `settled` with amount unchanged at 4000; a second refund refused `NEXUS_NOTHING_TO_REFUND`; and the cause was then frozen. Nothing persisted — verified after: 0 probe orders, 0 claims, 0 refund events, total events still 321.
- Verified: new `check_cancellation_cause.mjs` (18 tests) covering scenarios A–H. TWELVE mutations all caught — courier cause removed, customer cause forced, automatic refund restored, claim lifecycle omitted, claim settling without an event, claim settlement refunding the customer, duplicate claim, duplicate refund, unauthorized cause change, cause editable after resolution, a claim writing money, and the shop hint promising a refund again. `courier_claims` was added to the anonymous-write RLS probe.
- One NEW failure appeared and was chased down rather than waved through: `an anonymous caller cannot write to any tenant table` failed with a Cloudflare 522 on the `orders` probe. That test deliberately refuses to count a non-42501 answer as a pass, which is why it surfaced. Re-run twice, clean both times — ENVIRONMENTAL, not caused by the new trigger (which does nothing for a probe whose cause is absent). One PRE-EXISTING assertion in `check_return_policy` needed updating: it pinned `canonicalWallet(...) ?` as a bare ternary and the call site grew a second condition, so it was pinning the spelling rather than the rule.
- Suite 1270 total / 1259 passed / 5 stale pre-existing auth failures / 6 skipped — zero new. `npx tsc --noEmit` 0, `npm run build` 0, `npm run build:mobile` 0.
- Still open, and named: the shop-caused deposit decision above; and the claim UI offers every legal transition except `settled`, which is reachable only from a real courier settlement and is therefore not yet wired to the settlement screen.

---
## Returns — courier-caused resolution and the deposit exception (2026-09-22)
- Problem: `depositForfeitedOn` was a BOOLEAN, and `false` meant "refund, now, automatically". So a courier- or shop-caused return handed the customer their deposit back at the moment of confirmation — with nobody deciding, and nothing recording that a decision had been made. That is a blanket refund rule and it is wrong for the same reason the blanket forfeit was: it answers a question nobody has asked yet, because the customer has not said whether they still want the goods. The real sequence has days in the middle of it — courier causes the return → the shop claims compensation → the customer decides → ONLY THEN is the deposit resolved.
- Found while tracing it, and fixed: the same `!keepsDeposit -> refund` shape meant an EXCHANGE also refunded the deposit, so the customer got the money back AND the replacement order collected a second one. The third disposition moves nothing.
- Model: `depositDispositionOn(cause, movement)` returns three answers. `forfeit` (the customer walked away — Rule A, final, `revenue/forfeited_deposit`), `pending_resolution` (the shop or the courier caused it — the cash stays in the till under `revenue/deposit_pending_resolution`, visibly NOT final), and `none` (an exchange — the same money funds the replacement). A courier-caused return is never filed as a customer cancellation: no forfeit, no wasted-trip debt, and the fee is never theirs.
- Migration 038, applied: `deposit_refunded` joins the money-kinds branch of `insert_ledger_events` so only ADMIN and ACCOUNTANT may write one — it would otherwise fall to the ELSE branch, where a cashier could refund a deposit. And `refund_order_deposit(order, wallet, note)`, SECURITY **INVOKER** for the reason migration 032 gives: it needs a transaction boundary and a business rule, not privileges, so `select_orders` is the tenant gate, `insert_ledger_events` is the role gate and `insert_ledger_lines` is the licence gate — none of them re-implemented by hand.
- No `depositRefundedAt` column, deliberately. A flag on `orders` would be a second truth to keep in step with the money, and `orders` is UPDATE-able by three roles — so the guard on a refund would be editable by someone who cannot perform one. Instead the eligibility and the duplicate check are the SAME question asked of the append-only ledger: the refund reverses exactly what is standing on `deposit_pending_resolution`, so the balance IS the entitlement and a second call finds zero. A per-order `pg_advisory_xact_lock` makes that check atomic — without it two operators would both read a non-zero balance before either wrote.
- Compensation and deposit refund stay separate concepts and separate events: 40 owed to us by the courier, 300 owed by us to the customer, settled by different mechanisms. The refund touches NO courier account — asserted — because paying a customer must not quietly forgive the provider. The dialog says so on screen too.
- Replacement orders are linked, never merged: Order B is its own document with its own placement, delivery and shipping cost, and `orders.original_order_id` is the link. Nothing rewrites Order A, and the ledger cannot be updated by any client role anyway.
- Where the choice lives: in the INCIDENT, not in Settings. A «رد العربون = ON» toggle would be a way to configure an invalid financial policy. The button appears only on a confirmed, eligible return, opens its own dialog, and needs an explicit confirmation; declining writes nothing, because holding is what already happened.
- Runtime, against the disposable QA store inside a self-aborting DO block so NOTHING persisted (verified after: 0 probe orders, 0 probe events, `deposit_refunded` count 0, total events still 321). Held before refund 30000 piastres; after the refund held=0, wallet=−30000, ltv=−30000 and exactly ONE `deposit_refunded` event; a second call refused `NEXUS_NOTHING_TO_REFUND` with the count still 1; a customer-caused order refused `NEXUS_CAUSE_NOT_ELIGIBLE`; an unknown order refused `NEXUS_ORDER_NOT_FOUND`.
- Verified: new `check_courier_resolution.mjs` (14 tests) plus the updated `check_return_policy.mjs`. TEN mutations all caught — customer cancellation refunding, courier return treated as a cancellation, courier return auto-refunding again, refund without an eligible cause, the duplicate guard removed, the kind falling to the four-role branch, compensation booked as a customer charge, company exchange becoming customer-paid, the cause guard neutered, and the refund silently cancelling the courier claim. The harness initially reported four of them as "no match" — the files are CRLF and the needles were written with `
`, which looked like "no such code" when it meant "wrong line ending"; fixed and re-run. Three PRE-EXISTING suites encoded the old auto-refund and were corrected rather than deleted: `check_exchange` ("a courier-caused RTO refunds the deposit" → HOLDS it), `check_return_settlement` (two RTO cases), and the deposit assertions in `check_return_policy`.
- Suite 1252 total / 1241 passed / 5 stale pre-existing auth failures / 6 skipped — zero new. `npx tsc --noEmit` 0, `npm run build` 0, `npm run build:mobile` 0.
- Scope flagged, not hidden: the brief names only the COURIER case for the deposit exception. The same mechanism covers a SHOP-caused return, because both are "not a customer cancellation" and the alternative was leaving shop-caused on the blanket auto-refund this correction removes. If a shop-caused return should instead forfeit, it is a one-line change in `depositDispositionOn`.
- Not built, and named as such: a formal claim-status lifecycle (open → claimed → settled) with settlement matching. Compensation is traceable today as a `receivable_courier` line on an event whose `return_cause` is `courier`, and it settles at the courier batch — but there is no claim STATUS field and no per-claim settlement record. That is the remaining gap in §4 of the brief.
- The two screens were not driven at runtime: both sit behind `ProtectedRoute` and this pass holds no NexusCore login. The ledger and RPC proof above is executable; the UI wiring is asserted statically.

---
## Returns / Exchanges / Deposit — business-rule correction (2026-09-21)
- Problem: three defects in one policy, found by tracing cause → responsibility → ledger rather than reading labels. (1) `buildOrderCancelledLines` wrote `wallet −deposit` unconditionally — the customer cancels and the deposit is handed straight back, no cause asked, no income recognised. The forfeit path existed on every OTHER way an order comes back (`return_confirmed`, `rto_confirmed`, `depositForfeitedOn`, `revenue/forfeited_deposit`) and this one route out of the funnel consulted none of it. (2) Both confirm dialogs default the cause to `unknown`, and `shippingBorneBy("unknown","exchange")` resolves to `"customer"` — so an operator who simply pressed تأكيد billed the customer for a swap the shop may have caused. That is the blanket "exchange = customer pays" rule surviving as a DEFAULT rather than as a line of code. (3) The exchange picker showed «العميل», which reads as *the customer asked* — the requester, not the cause.
- Already correct, and left alone: the cause axis itself. `return_cause` (migration 026), `shippingBorneBy`, `depositForfeitedOn`, `countsAsWastedTrip` and the `feeBorneBy` branch in `buildReturnConfirmedLines` already implement cause-decides-responsibility on the return and RTO paths. No second responsibility field was created, no parallel return system, no migration, no schema change, no RLS change.
- Fix (1): `buildOrderCancelledLines` no longer accepts `depositAmount` and no longer writes a wallet line for it. The disposition is EXPLICIT — `forfeitedDeposit` (the customer walked away; `revenue/forfeited_deposit` + LTV, no cash movement) or `refundedDeposit` (the order DOCUMENT was refused after placement banked the money, so no order ever existed and it comes back out). Mutually exclusive, no default, and the old field is gone from the type so an old call site cannot compile into the old behaviour. Found while fixing it: a pre-`depositWallet` order holds an amount with no wallet and never wrote a wallet line at placement, so forfeiting one would recognise income against cash the ledger never saw — the call site gates on the wallet, exactly as the old refund did.
- Fix (2): `blockingCauseReason` refuses an unclassified movement — on the button AND re-checked in both handlers, because a dialog can sit open while state moves and the handler is where the event becomes permanent. `shippingBorneBy`'s `unknown` fallback is deliberately NOT changed: every pre-026 row is `unknown` and re-deciding them would move historical figures. It stays as the reading of history; the guard stops new history being written into it.
- Fix (3): `EXCHANGE_CAUSE_LABELS` — «تغيير رغبة العميلة» / «خطأ من المندوب / شركة الشحن» / «خطأ من المحل أو عيب في المنتج». Company fault and product defect are ONE cause deliberately: they have one payer, and splitting them invites an operator to file a defect under the customer. Both confirm surfaces now show «المسؤول المالي» (على المحل / تعويض من شركة الشحن / على العميلة) before تأكيد.
- Settings: added a fixed-rule panel — stated, never switchable, because a toggle there would be a way to configure an invalid financial policy. And `depositMandatory` now actually gates submission: it had been described to the owner as "disabling the submit button" since it was added and nothing read it, so a shop believed every online order carried a stake and none of them had to.
- Runtime, by executing the real builders over all nine scenarios (EGP; deposit 200, goods 500, fee 40): S1 cancel → `stock +300, revenue/forfeited_deposit +200, customer_ltv +200` and NO wallet line; S1b rollback → `wallet −200`, no revenue; S2/S5/S8/S9 shop → `expense/shipping_return +40`, no receivable; S3/S6 courier → `payable +40` AND `receivable +40`, nets to zero for the shop, `compensationExpectedFrom = courier`; S4 customer return → deposit kept, wallet out 300 not 500, wasted trip counted; S7 voluntary exchange → customer bears, no expense, deposit untouched. S6 and S7 produce IDENTICAL lines — both are non-shop pass-throughs against the same courier account — and that is correct: the distinction is carried by `return_cause` on the append-only event, not by the amounts.
- Data audit, read-only: 12 orders carry a real cause, 22 are still `unknown` (9 of them returned), 1 exchange record is `unknown`, and `revenue/forfeited_deposit` already holds 5 lines / EGP 1,300. The cancellation defect NEVER FIRED — the single `order_cancelled` event in the whole database carries no wallet line, so no customer was ever wrongly refunded and there is nothing to unwind. Reclassifying the historical `unknown` rows is a RECOVERY PROPOSAL, NOT EXECUTED: their accounting is internally consistent and re-deciding them would move figures in an append-only ledger.
- Verified: new `check_return_policy.mjs` (21 tests, all nine scenarios asserted at the LEDGER not the label), mutation-guarded six ways — company exchange → customer pays, courier return → customer pays, the cause guard neutered, the handler re-check removed, deposit refunded on cancellation, and voluntary exchange → shop-paid each fail it. Five PRE-EXISTING tests encoded the old rule and were corrected one by one, not deleted: three in `check_deposit_lifecycle`, one in `check_order_money` (now asserts the forfeit AND keeps a rollback-refund case), one in `check_exchange` (the compensation path, which legitimately still refunds). `check_exchange`'s "an exchange trip is NEVER the shop's expense" was retitled to the voluntary case — it asserted the blanket rule, so the correct model could not have landed without it objecting. Suite 1237 total / 1226 passed / 5 stale pre-existing auth failures / 6 skipped — zero new. `npx tsc --noEmit` 0, `npm run build` 0, `npm run build:mobile` 0.
- OPEN BUSINESS DECISION, not resolved here: on a return the SHOP or the COURIER caused, the deposit is currently REFUNDED. The brief is internally inconsistent — rule 3A says a company-caused return is "NOT charged to the customer" (forfeiting IS charging them) while Scenario 2 says "deposit is NOT refunded". Current behaviour kept as the conservative reading: keeping a customer's money on our own mistake is profiting from it, and taking money is an irreversible entry where giving it back later is not. See `docs/RETURN_EXCHANGE_POLICY.md` §6.
- Not verified at runtime: the two screens themselves. Both sit behind `ProtectedRoute` and this pass holds no NexusCore login; the ledger proof above is executable and the UI wiring is asserted statically.

---
## Desktop — P0 Wave 1: the four production blockers (2026-09-21)
- Problem: `docs/DESKTOP_PRODUCT_AUDIT.md` proved four blockers between a building app and a shop that can use it. P0-1 the production URL sat behind Vercel Authentication, so an ordinary visitor met a Vercel login and never saw NexusCore's. P0-2 the header hardcoded «سارة المصري» / «مدير النظام». P0-3 المرتجعات and ربط المتجر الإلكتروني were hidden from the sidebar by a per-browser `localStorage` flag defaulting to `false`. P0-4 `useRealtimeSync` called `useSessionReconciliation()` and threw the answer away, so `ProtectedRoute` gated on a boolean in `localStorage`.
- P0-1 — `ssoProtection` set to `null` on `nexuscore-web1` via the Vercel API; the project now reports `enabled: false`. No repository change, and application authentication was NOT touched — Supabase Auth + RLS remains the only boundary, so what the change exposes is the login screen, not the data. Verified anonymously with no cookie jar: `/` and `/login` both 200 with `<title>NexusCore`, no `_vercel_sso` cookie, no redirect to vercel.com; and rendered in a browser, the URL paints «تسجيل الدخول».
- P0-2 — the audit's severity was WRONG and is corrected in the doc: `dashboard/Header.tsx` has one importer, `src/routes/index.tsx`, a TanStack file-route that `main.tsx` never mounts, so that header never rendered. The real defect was adjacent — the SHIPPED shell `layout/Layout.tsx` showed no identity at all, so nobody on a shared till could see whose session they were in. One new `layout/SessionIdentity.tsx` now serves both headers (two copies is how the fiction survived in the one nobody read): username from the session, role from `ROLE_LABELS[toAppRole(userRole)]`, and the System Owner badge ALONGSIDE the store role, never instead of it. Three dead controls deleted rather than wired — a search input with no handler, a bell with an unconditional unread dot, and a قطاعي/جملة toggle nothing read.
- P0-3 — both module flags default `true`, plus `version: 1` and a `migrate`. The default alone fixes nothing: `persist` rehydrates OVER the initializer, so every browser that ever opened the app still held the old `false`. The two values are forced because the old `false` carries no information — it is what the store wrote on first run, so "switched off deliberately" and "never touched" are the same byte, and a visible module a shop ignores costs less than an invisible one. The flags were NOT deleted: both toggles stay in الإعدادات → عام, and a switch-off after this ships is preserved.
- P0-4 — four changes, one fact. `useRealtimeSync` returns the verdict and `App` hands it to `<ProtectedRoute sessionState={…} />`, which holds on `"checking"` and refuses anything else. The `store_members` read moved out from behind `if (!isAuthenticated)` so the role is re-asked on EVERY boot — that conditional is why a demoted user kept an ADMIN sidebar (`KNOWN_LIMITATIONS.md` #17). A membership that is definitively gone ends the session; a FAILED query does not, because `maybeSingle()` reports "no row" and "network died" the same way and signing someone out over a dropped packet throws away their work while Postgres refuses them regardless. System Owner is exempt — they hold no membership by design.
- Found while fixing P0-4, and fixed: boot hydration and the `global-sync` channel both fired before the session existed. Realtime applies RLS using the token the socket JOINED with, so a channel opened early joins as `anon` and then silently delivers nothing for its whole life — no error, no reconnect. Mobile already gated `useMobileRealtime` for exactly this. Both now wait for `"authenticated"`; the hook issues no redirect, sets no role and grants nothing.
- Runtime, in a real browser against the live project — the profile happened to hold a genuine stale session (`isAuthenticated: true`, `userRole: "ADMIN"`, JWT expired the previous day): a cold load held on «جارٍ التحقق من الجلسة…» then resolved to `/login` with the flag, session and username cleared; forging `isAuthenticated: true` + `userRole: ADMIN` + `isSystemOwner: true` and opening `/products` still landed on `/login`, and `isSystemOwner` proved never to be persisted at all; an unauthenticated boot made ZERO `supabase.co` requests for tenant tables and logged no `[Hydrate]` line. Seeding the pre-fix `feature-storage` blob (`version 0`, both flags `false`) and reloading produced `version 1` with both module flags `true` and the other three untouched; seeding a deliberate post-migration switch-off left it off. `سارة المصري` is in `git show HEAD:…/Header.tsx` and absent from the built `dist/` bundle.
- Verified: new `check_desktop_p0.mjs` (23 tests, 22 + 1 opt-in live), mutation-guarded seven ways — removing the `"checking"` hold, returning a constant verdict, opening realtime early, signing out on a dropped packet, flipping either module flag off, dropping the migration, and hardcoding the role label each fail it. One mutation ESCAPED on the first run (a file-wide `returnsEnabled: true` match was satisfied by the `true` inside `migrate`) and the assertion was tightened to the initializer and re-run. One PRE-EXISTING test repointed: `check_invite_staff` matched `<ProtectedRoute />` verbatim and now matches the tag name, since the guard takes a prop. Suite 1213 total / 1202 passed / 5 stale pre-existing auth failures / 6 skipped — zero new failures. `npx tsc --noEmit` 0, `npm run build` 0, `npm run build:mobile` 0.
- Not done, deliberately: the 5 stale auth tests (audit P1-7, step 5 of the execution order) are untouched — they are not caused by this wave and repointing them is P1. No Mobile source changed. No migration, no RLS, no ledger code, no production data. Two P0-2 runtime scenarios are ASSERTED, NOT OBSERVED — «a real user sees their own name» and «changing user does not retain identity» need a NexusCore login this pass does not hold (`pwd.txt` is `owner`, the removed local-auth backdoor string, not a Supabase credential) and creating accounts was out of bounds. Someone with a QA account should confirm them.

---
## Mobile — P2-8: surface swallowed read errors (2026-09-21)
- Problem: four Mobile reads could fail with nothing on screen to say so, and each failure told a DIFFERENT lie rather than merely losing a spinner. `waitingError` → «لا توجد طلبات نشطة تنتظر هذا المنتج» when nobody had actually been asked, which is the sentence someone reads before deciding not to reorder. `financialsError` → the whole الملخص المالي card VANISHED, so the screen looked like a customer who had never traded. `suppliersError` → an empty picker indistinguishable from "no suppliers yet", leaving «+ مورد جديد» as the only apparent option — which is exactly the duplicate-supplier bug `lib/receiving/suppliers.ts` exists to stop, splintering a supplier's `payable_supplier` across copies. A fourth was found while fixing them: the order-history retry was wired to `setOrdersPage({ loading: true })`, a spinner that never resolved because nothing was fetched.
- Fix: each loader hoisted out of its effect into a `useCallback` so the error state can ask again through the SAME reader — no second fetch path, and the call-site count is asserted. Every section now renders loading / error+retry / empty / success as distinct states, with the error branch checked BEFORE the empty branch so a failure can never be reported as an answer.
- Financial safety: on failure `financials` is cleared rather than left holding the previous customer's numbers, and the section shows an error instead of figures. No zero defaulting, no client-side compensation — asserted by a test that forbids `financials?.x ?? 0`.
- Supplier safety: while the list is unknown the picker is disabled AND «+ مورد جديد» is withheld, because choosing "new" against a list that merely failed to load is how duplicates get minted. `canSave` also gained `&& !suppliersError`, so a receipt cannot be committed against a supplier list nobody could read.
- Runtime, by injecting controlled failures into `fetch` and navigating client-side so the stub survived: financials showed «تعذّر تحميل الملخص المالي لهذا العميل.» and «تعذّر تحميل سجل الطلبات.» with retries and NO zero figures on screen, then retry restored ١٬٥٠٠٫٠٠ revenue / 28 orders / the warnings block; suppliers showed its error with the picker disabled, «+ مورد جديد» absent and توريد blocked, then retry restored «اختر المورد…»; waiting orders showed «تعذّر تحميل الطلبات المنتظرة.» and did NOT claim nobody was waiting, then retry restored the genuine empty state — the same sentence, now only when it is true. Owner cockpit re-checked against the database afterwards: محمود ٢٬٣٣٠٫٠٠ = 2330.00, the orphan ٢٬٠٠٠٫٠٠ = 2000.00, and 36 orders / 321 ledger events / 13 invoices unchanged.
- Verified: new `check_mobile_read_errors.mjs` (13 tests), mutation-guarded eight ways — restoring the "nobody is waiting" lie, keeping a partial list, keeping the previous customer's figures, reverting the history retry to a flag flip, re-offering «+ مورد جديد», allowing a commit with an unknown supplier list, dropping the error reset, and defaulting a failed financial read to zero each fail it. Three PRE-EXISTING tests needed correcting, all the same pattern — an assertion that could not tell prose from code or that pinned an expression which legitimately grew: `check_mobile_m2_fixes` now strips comments before forbidding ledger-account names, and two `check_mobile_p2_hardening` assertions stopped pinning the `canSave` tail and stopped matching their own file's mention of the old home-screen name. Suite 1180 passed / 5 stale pre-existing auth failures / 5 skipped. Mobile+security subset 275 passed / 0 failed / 3 credential-skipped. `npx tsc --noEmit` 0, `npm run build` 0, `npm run build:mobile` 0.
- Not done, deliberately: P2-9 lint debt. No broad `src/mobile` reformat, no global `no-unused-vars`, no `any` sweep. Still ONLINE-ONLY — no queue, no local storage, no local write was added to survive a failed read, and `commitReceipt` remains the only write path. No production financial history was modified.

---
## Mobile — P2 hardening (2026-09-21)
- Problem: seven P2 findings from the Mobile Product Completion Audit. Each was a way the app was quietly lying — a queue row that invented its own status word, error screens with no way out, lists that looked live while the connection was gone, and a توريد button that stayed enabled so the operator could discover they were offline by filling in a receipt and pressing submit.
- P2-1 — `mobileHomeReader` hardcoded `statusLabelAr: "قيد الإجراء"` while the taxonomy and the Orders screen said «قيد الانتظار» for the same order. It now calls `resolveOrderStatus(order.status)`; the shipments queue hardcoded its label too and now calls `resolveShipmentStatus`. Runtime: the home queue reads «قيد الانتظار» and the stale word is gone from `src/` entirely.
- P2-2 — `useMobileEntity` had no `reload`, so تفاصيل الطلب / المنتج / العميل rendered `ErrorState` with no retry and a failed read was terminal. The hook now returns `reload`, which re-runs the SAME reader (there is exactly one `await reader()` in the file), guarded by `inFlight` against a double tap and by a generation counter so a late answer cannot repaint a record the user has navigated past. All three screens pass `onRetry={reload}` and gained a back affordance in their terminal states.
- P2-3 — new `useIsOffline` SUBSCRIBES to `online`/`offline`. Eight screens previously asked `typeof navigator !== "undefined" && !navigator.onLine` inline, which only re-evaluates when something else re-renders, so losing the connection was never announced; الرئيسية had already written the listener pair by hand and that is the shape this extracts. All eight now use the hook, and the four screens with no connection state at all (the three detail screens + المالية) have one. The Owner screen gates EVERY branch behind it: a money screen with no connection shows no figures, not the last ones it happens to be holding.
- P2-4 — restock's `canSave` gained `&& !offline`, plus a title explaining why the button is unavailable. The write already failed safely; what it did not do was say so before the tap. No draft, no queue, no local write — `commitReceipt` via `executeQuickRestock` remains the only write path.
- P2-5 — the two named dead store imports are gone. A `tsc --noUnusedLocals` pass over the touched files found forty more and those were removed too; `@typescript-eslint/no-unused-vars` is switched **off** in `eslint.config.js`, which is why they had survived. Four unused *state* variables were deliberately kept: deleting `waitingError`, `financialsError`, `financialsLoading` and `suppliersError` would remove error paths rather than surface them, so they are recorded as new finding P2-8 instead.
- P2-6 — `MobileHomePlaceholder` → `MobileHomeScreen`; it has not been a placeholder for some time. All references updated, none remain.
- P2-7 — the persona doc's stale "pending/processing" line now says `pending` and names `orders_status_check` rather than just dropping the word. The abandoned 20240608 migration is left exactly as it was, and a test asserts it stays that way — editing history to match the present is how the next audit loses the evidence.
- Verified at runtime on the live store: the home queue label; a detail screen switching to «لا يوجد اتصال بالإنترنت» on an `offline` event with NO other re-render trigger, and back on reconnect; the restock button going `disabled` with «لا يوجد اتصال بالسحابة» and clearing on reconnect; and the full retry cycle under a controlled failure — PostgREST reads stubbed to reject, the screen showed «إعادة المحاولة», the network was healed, two rapid taps produced one retry cycle, and the product rendered. المشتريات and العملاء still render. A fresh tab with a clean console buffer shows zero errors (the `MobileHomePlaceholder` 404 and a hooks dep-size warning seen earlier were HMR artifacts of the rename, cleared by a dev-server restart).
- Verified: new `check_mobile_p2_hardening.mjs` (21 tests), mutation-guarded seven ways — home inventing a label, the loader losing `reload`, a screen dropping its retry, connectivity going back to render-time sampling, restock submittable offline, restock queueing locally, and the Owner screen rendering figures while offline each fail it. One PRE-EXISTING test needed updating: `check_owner_mobile.mjs` pinned the exact old branch expressions, and `offline` is now a fifth mutually-exclusive state ahead of them. Suite 1167 passed / 5 stale pre-existing auth failures / 5 skipped. Mobile+security subset 262 passed / 0 failed / 3 credential-skipped. `npx tsc --noEmit` 0, `npm run build` 0, `npm run build:mobile` 0.
- Still ONLINE-ONLY: no offline queue, no local write, no conflict engine, no reconciliation. Knowing the connection is gone exists so the app can SAY so and refuse to pretend. BD-1..BD-7 remain untouched business decisions, and Mobile is NOT marked complete — P2-8 and P2-9 are now open.

---
## Mobile — P1 Wave 2: المشتريات, الإعدادات and sign-out (2026-09-21)
- Problem: the last two Mobile placeholders. P1.4 — `/purchasing` was a قريباً card while being ACCOUNTANT's primary bottom-nav destination. P1.5 — `/preferences` was a قريباً card offered to all five roles.
- Correction to the audit: it claimed "sign-out has no home". That was wrong — `MobileMoreSheet` has called the canonical `signOutCurrentSession()` since it shipped. Preferences now offers the same action from the same function; it did not add a missing capability.
- Purchasing — what was actually missing was the READ, not the write. `commitReceipt` is the ONE way a supplier receipt reaches the database, and `/restock` has called it from the phone since توريد سريع shipped; the persona architecture recorded Purchases as "➖ written ✅, not read". So the new screen adds no purchasing policy and no second accounting path: الفواتير lists `purchase_invoices` (including the document's own paid/partial/unpaid status, not a recomputed one), الموردين reads `SUM(payable_supplier)` from the ledger, and توريد جديد hands straight over to the existing `/restock` flow. Owed is deliberately NOT `totalAmount − paidAmount`: `supplierTotals.ts` says the owed figure lives in the ledger "so it stays right no matter which screen moved the money", and a supplier payment made on Desktop moves the ledger without touching an invoice row.
- Preferences — persona-neutral by design: account identity, light/dark through the existing `useThemeStore` + `applyTheme`, app version, and sign-out. Store settings, members, branches and licence administration are ADMIN-on-Desktop and marked ❌ for every persona in the capability matrix, so none of them is behind a screen all five roles can open. No System Owner surface. No offline, sync, conflict or local-accounting setting — there is nothing to configure because the server is the authority.
- Also: `purchase_invoices` joined `MOBILE_REALTIME_TABLES`, so the new read updates live like every other mobile list; both nav entries lost `isImplemented: false`, so the More sheet no longer shows them as قريباً.
- Verified at runtime against the live store. Purchasing: 11 real invoices render with supplier «محمود», correct dates and مسددة / مسددة جزئياً / آجل badges with the remaining amounts; الموردين shows «محمود ٢٬٣٣٠٫٠٠ ج.م.» — the SAME figure the Owner cockpit shows, because both read the same ledger account. Preferences: account card, theme toggle (light `rgb(248,250,252)` → dark `rgb(11,17,33)`, `dark` class applied, restored), version 1.0.0. Sign-out on a real authenticated session: Supabase session `hasSession: false` afterwards, redirect to `/login`, a direct hit on `/owner` bounced to `/login`, and two Back presses both stayed on `/login`.
- Authorization proven in Postgres, not by route hiding: attempting a `purchase_invoices` insert as ADMIN was allowed (rolled back, 0 rows persisted), as POS_ECOMMERCE was refused `42501`, and as a foreign ADMIN refused `42501` — matching `has_role(store, ADMIN, ACCOUNTANT)`. `canAccess` resolves purchasing to exactly ADMIN + ACCOUNTANT, and MODERATOR's stated capability set still omits it, which is what keeps the one mobile write out of a read-only persona's reach. No mobile screen or reader carries a `store_id`.
- NOT done, and why: no purchase was manufactured in the production ledger. `ledger_events` is append-only by policy (`no_update`, `no_delete`), so a QA purchase could not have been cleaned up afterwards, and the brief forbids modifying production financial history. The write path is unchanged by this wave and remains covered by `check_quick_restock.mjs`.
- Verified: new `check_mobile_purchasing_preferences.mjs` (17 tests) — role matrix behaviourally through `canAccess`, no write path in the purchasing screen, owed-from-ledger, document status not recomputed, no `store_id` on the client, canonical sign-out, `replace` so Back cannot restore, double-press guard, and the absence of privileged and offline settings. Mutation-guarded seven ways (React-derived debt, a second write path, local-only logout, push instead of replace, licence surfaced, Moderator granted purchasing, placeholder restored) — each fails it. One PRE-EXISTING test needed updating: `check_mobile_phase_b.mjs` asserted `MobileDeferredScreen` was routed, which was true only while these two screens were placeholders. Suite 1146 passed / 5 stale pre-existing auth failures / 5 skipped. Mobile+security subset 280 passed / 2 failed (both from the stale five) / 3 credential-skipped. `npx tsc --noEmit` 0, `npm run build` 0, `npm run build:mobile` 0.
- Roadmap: the open "one offline sells + one online sells" multi-device item is now recorded in `NEXUSCORE_PLAN.md` as OUT OF PRODUCT SCOPE — ONLINE-ONLY, not as deferred work. Mobile is NOT marked complete, and the UX/UI pass is NOT marked complete.

---
## Mobile — P1 Wave 1: realtime, refresh, Owner labels (2026-09-21)
- Problem: three findings from the Mobile Product Completion Audit. P1.1 — Mobile never mounted any realtime, and had no polling or refetch-on-focus, so a Moderator watching the orders queue never saw a new order until they navigated away and back. P1.2 — the تحديث buttons on Orders, Customers and Shipments had no `onClick` at all, which on an app with no realtime meant the only refresh path silently did nothing. P1.3 — the Owner cockpit printed raw subject ids (`26dfe561-…`, `default`, `cert-courier`) because all three `SubjectList` call sites omitted the `labelOf` prop that already existed.
- Fix: new `useMobileRealtime` opens ONE Supabase channel for the whole app and dispatches a `mobile-realtime` window cue; `useMobilePagedQuery` gained `watch` (re-run this reader when that table changes) and a `refresh()` that swaps rows in place. `useSubjectNames` maps supplier and courier ids to names through the existing canonical readers (`readSuppliers`, `readMobileCouriers`).
- Deliberately NOT `useRealtimeSync`: that hook is inseparable from Desktop's state model — every handler merges into a Zustand store Mobile never reads, and it calls `hydrateAll()` on boot. Mobile pages straight from Supabase and never hydrates, so mounting it would have run a destructive boot hydration and merged rows into collections no mobile screen renders. The new layer owns no merge logic at all: a socket event is a cue to re-run the screen's own reader, so the reader keeps the filter, the ordering and the paging, and RLS scopes the result a second time. No second state path, no client-side accounting.
- Two real bugs were found only by driving the running app, and both are pinned by tests. (1) The channel opened at root mount joined BEFORE the session was restored; Realtime applies RLS using the token the socket joined with, so it reported `state: "joined"` with every binding present and delivered nothing — a hand-made channel opened after login received the same events on the same socket. It is now gated on `sessionState === "authenticated"`. (2) Rejoining a FIXED topic after StrictMode's unmount/remount left the server bound to the discarded instance's binding ids, so the topic is now unique per open and the close is deferred.
- Scope: subscribes to `orders`, `products`, `customers`, `ledger_events` — the tables Mobile actually reads. Deliberately NOT `transactions` or `expenses` (Mobile reads neither table; the money screens read the ledger) and NOT `ledger_lines` (several rows per event, and `ledger_events` already signals the same movement). `couriers` is read but absent from the `supabase_realtime` publication, and no schema change was made.
- Improvement: Mobile lists now update live; تحديث works everywhere and keeps the rows and the scroll position on screen instead of blanking into a skeleton; the Owner sees «محمود» and «CERT Courier» instead of UUIDs, and an id that resolves to nothing renders «غير معروف» with the raw id kept as the hint rather than being hidden or given an invented name.
- Verified: runtime, against the live store with disposable QA data that was deleted afterwards (36 orders before and after, product name restored). An inserted order appeared with no refresh; updating it to `shipped` REMOVED it from the pending queue live and it appeared under «في الطريق» with its new name — proof the cue re-runs the reader rather than merging a row past its filter. Product and customer changes propagated the same way, each with exactly one row and no duplicates. The refresh control: 3 rapid presses produced 2 reads (duplicate suppressed), rows stayed at 3 throughout, and the busy state was observed via MutationObserver. Exactly one `mobile-sync` channel existed despite StrictMode. New `check_mobile_realtime.mjs` (26 tests) covers mounting, the auth gate, single subscription, the table list, payload-never-applied, all four refresh controls and the Owner labels — mutation-guarded seven ways (dropping the auth gate, a fixed topic, an extra table, un-wiring a button, dropping `labelOf`, faking a name, dropping the duplicate guard) each fail it. Suite 1129 passed / 5 stale pre-existing auth failures / 5 skipped. Mobile+security subset 224 passed / 0 failed / 3 credential-skipped. `npx tsc --noEmit` 0, `npm run build` 0, `npm run build:mobile` 0.
- Not changed: no RLS, schema, migration, auth model, licence, role model, ledger or order-lifecycle change. Still ONLINE-ONLY — no offline queue and no offline write path was added.

---
## Mobile — Product Completion Audit (2026-09-21)
- Problem: Previous phases certified specific systems (persona security, owner cockpit, realtime, status contract) but nobody had asked whether Mobile is complete as a PRODUCT. "Screen exists" had never been tested against "full business flow works".
- Fix: Audit only — no source, migration, schema or production data changed. Findings recorded in `docs/MOBILE_PRODUCT_AUDIT.md`.
- Headline: Mobile is a read-only operations cockpit with exactly ONE write (`/restock` -> `commitReceipt`); `src/mobile/` contains zero direct table writes. 3 P0 items, 5 P1 items, 7 P2 items, 8 business decisions. Transparent completion: 21.5 / 26 decided capabilities = 82.7%.
- P0: the mobile PWA has no production deployment at all (`vercel.json` builds `npm run build` into `dist` — the desktop app — and `build:mobile`/`dist-mobile` is never served); and the service worker's `navigateFallback: "/index.html"` names a file the mobile build does not emit (its only HTML output is `mobile/index.html`).
- P1: Mobile never mounts `useRealtimeSync`, so it has no realtime, no polling and no refetch-on-focus; three refresh buttons (Orders, Customers, Shipments) have no `onClick`, which is the only refresh path on an app with no realtime; the Owner cockpit prints raw supplier/courier ids because the three `SubjectList` call sites omit the `labelOf` prop that already exists; `/purchasing` is a placeholder that is ACCOUNTANT's primary bottom-nav tab; `/preferences` is a placeholder offered to all five roles and sign-out has no home.
- Offline: recorded as OUT OF SCOPE — ONLINE-ONLY PRODUCT, explicitly excluded from P0/P1. Verified the build already matches that decision: shell-only precache, no Supabase response caching, no background-sync queue, and `commitReceipt` throws with no local queue so an offline write cannot fake success.
- Verified: security clean with no gaps, all at runtime against the live DB by simulating sessions (`request.jwt.claims` + `SET ROLE authenticated`) rather than reading source — foreign member sees 0 rows across all six tables Mobile reads, anon sees 0, own member sees 34/7/8/489/167/1 with the 2 orders of an unrelated store invisible; `owner_financial_summary` allows ADMIN and refuses POS_ECOMMERCE, foreign ADMIN and anon each with the right 42501; MODERATOR appears in zero of the 27 write policies; every write policy is `has_role`-gated with none `is_store_member`-only. Runtime UI verified on the mobile dev server against the live store: home, orders, shipments, shortages and owner all render real data, and the e1025a3 shipments fix confirmed live — "جاهز للشحن" now lists all 3 pending orders.
- Not changed: no production data modified; no migration created; historical migrations untouched; the offline-selling item was not implemented and remains intentionally excluded.

---
## public.mobile_shortages — Remove stale `processing` from the RPC (2026-09-21)
- Problem: After e1025a3 cleared `processing` out of every application path, one reference survived inside the `mobile_shortages` SECURITY DEFINER function: `o.status IN ('pending', 'processing')`. The CHECK constraint makes that second value inert today, which is precisely why it had escaped — but a predicate that is harmless only because of a constraint somewhere else starts selecting again the day that constraint moves.
- Fix: Migration 037 narrows the predicate to `o.status IN ('pending')`. Applied to the live project as `mobile_shortages_status_contract`. Nothing else in the function changed.
- Improvement: None visible — the live CHECK already guaranteed zero rows could match `processing`. This closes the contract rather than altering behaviour.
- Preserved (re-verified on the live function after the replace): `SECURITY DEFINER`, `STABLE`, `search_path = public, pg_temp`, owner `postgres`, grants `{postgres=X, authenticated=X, service_role=X}` (CREATE OR REPLACE keeps a function's ACL), the `has_role` gate over ADMIN/ACCOUNTANT/ECOMMERCE_ONLY/MODERATOR with its load-bearing COALESCE, all three `store_id = p_store` tenant predicates, and the eight-column output shape.
- Verified: before writing 037, the normalised live definition and migration 033's copy both hashed to `9f244a14ca0a4b4f85811f26be8419fc`, proving 033 was a faithful record of production; 037 was then generated from that block with one substitution, and the new live definition and the repo file both hash to `a47cac42681c21447cd552ffc8ebe8c6`. `check_order_status_contract.mjs` extended by 4 tests asserting the filter is exactly `['pending']`, that no rejected or non-`pending` canonical status appears, that 037 equals 033-with-one-substitution, and that the guard/tenant scope/output shape survived. Mutation-guarded four ways — reintroducing `processing` fails 3 tests, dropping a `store_id` predicate fails 2, downgrading `SECURITY DEFINER` fails 2, adding `'shipped'` fails 3. Suite 1103 passed / 5 stale pre-existing auth failures / 5 skipped. Security+lifecycle subset 89 passed / 0 failed / 3 credential-skipped. `npx tsc --noEmit` 0, `npm run build` 0, `npm run build:mobile` 0. Order counts unchanged at 36 (returned 20, delivered 12, pending 3, shipped 1) — no production data was modified.

---
## orders.status — One status vocabulary, matching `orders_status_check` (2026-09-21)
- Problem: Mobile filtered, labelled and counted a sixth order status, `processing`, that Postgres has never accepted. Two real failures came out of it: the wholesale invoice screen WROTE `status: "processing"` when creating a shipping order, so "إنشاء طلب شحن" came back 23514 and created nothing; and the mobile Shipments screen's default "جاهز للشحن" stage filtered on `processing`, so it was permanently empty while the orders that are actually ready to ship (`pending`) were excluded from the shipments read entirely. Nothing caught it because `EcommerceOrderStatus` is `any`.
- Fix: Replaced every `processing` in an `orders.status` position with the canonical `pending` — the wholesale write, `shortages.ts` OPEN_STATUSES, `homeComposer` OPEN_ORDER_STATUSES, five filters/counters in `mobileReaders.ts`, the Shipments "ready" stage, and the Orders filter chip — and removed the `processing` entry from `ORDER_STATUS_TAXONOMY`. `ShippingSelector` (`ShippingInfo.status`) and `integrations.server.ts` (`online_orders`) are separate vocabularies and were left alone.
- Improvement: Wholesale invoices can create shipping orders again, and the mobile "جاهز للشحن" tab shows the orders waiting to go out.
- Not changed: `orders_status_check` itself. `processing` was never a business state — `orderLifecycle.ts` has no such node and migration 012 records the live lineage as pending/shipped/delivered/returned + cancelled. It came from `supabase/migrations/20240608_database_schema.sql`, an abandoned schema that also declares `completed`; Postgres rejects both. The `mobile_shortages` RPC still reads `IN ('pending','processing')`, which the CHECK makes a provable no-op — left untouched rather than redeploy a SECURITY DEFINER function for zero behaviour change.
- Verified: live DB probed in a rolled-back subtransaction — `processing` and `completed` → 23514, the canonical five accepted, 0 rows left behind; production holds 36 orders (returned 20, delivered 12, pending 3, shipped 1) and no `processing`. New `scripts/check_order_status_contract.mjs` (5 tests) pins the DB CHECK, the desktop `STATUS_META` and mobile `ORDER_STATUS_TAXONOMY` to the same five and fails on any source file that quotes a rejected status — mutation-tested by reintroducing the wholesale write. `check_mobile_phase_c.mjs` updated: it asserted the stale `processing`. Suite 1099 passed / 5 stale pre-existing auth failures / 5 skipped. `npx tsc --noEmit` 0, `npm run build` 0, `npm run build:mobile` 0.
